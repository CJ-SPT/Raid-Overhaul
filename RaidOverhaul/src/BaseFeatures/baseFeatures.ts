...
export class Base {
	...
	public raidChanges(modConfig: configFile): void {
		...
		if (modConfig.Raid.ReduceFoodAndHydroDegrade.Enabled) {
			...
			this.globals().Health.Effects.Existence.HydrationDamage = modConfig.Raid.ReduceFoodAndHydroDegrade.HydroDecay
		}
	...
	public traderTweaks(modConfig: configFile): void {
		...
		if (modConfig.Trader.RemoveFirRequirementsForQuests) {
			for (let q in this.quests()) {
				let quest = this.quests()[q]
				if (quest && quest.conditions && quest.conditions.AvailableForFinish) {
					let availableForFinish = quest.conditions.AvailableForFinish
					for (let requirement in availableForFinish) {
	...
	public weatherChanges(modConfig: configFile) {
	...
	public addReqShopInsurance(): void {
		const InsuranceConfig = this.ref.configServer.getConfig<IInsuranceConfig>(ConfigTypes.INSURANCE)
		InsuranceConfig.returnChancePercent["Requisitions"] = 95
	}
	//#endregion
}
