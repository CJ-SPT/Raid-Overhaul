...
export class Utils
{
    ...
    public profileBackup(modName, sessionID: string, path, profile: IAkiProfile, randomUtil): void
    {
    ...
    private addNewMastery(localeMasteryName: string, masteryTemplateItemID: string): void
    {
	...
	{	
		for (const item in this.ref.tables.templates.items) {
            ...
            {
				const slotsId = this.ref.tables.templates.items[item]._props.Slots[slots]._props.filters[0].Filter;
    ...
    public addToStaticLoot(lootContainers: string[], itemToPush: string, probability: number): void
    {
    ...
    public createClothingTop(clothingTopGen: ClothingTopSettings): void
	{
		...
		this.ref.tables.templates.customization[clothingTopGen.NewOutfitID] = newTop;
	...
	{		
		...
		this.ref.tables.templates.customization[clothingBottomGen.NewBottomsID] = newBottom;
